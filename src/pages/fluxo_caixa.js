...
async function carregar() {
  const [{ data: movs }, { data: clientes }, { data: produtos }] = await Promise.all([
    supabase.from("caixa_movimentos").select("*")
...
function render(container) {
  ...
  container.querySelector("#btn-entrada").addEventListener("click", () => abrirModalLancamento(container, "entrada"));
  container.querySelector("#btn-saida").addEventListener("click", () => abrirModalLancamento(container, "saida"));
  container.querySelector("#btn-importar").addEventListener("click", () => abrirModalImportar(container));
...
function abrirModalLancamento(container, tipo, dados = {}, ctx = null) {
  ...
  mDesc.addEventListener("input", () => {
    ...
    const matches = state.produtos.filter(p => p.nome.toLowerCase().includes(q)).slice(0, 7);
  ...
  area.querySelector("#btn-add-prod").addEventListener("click", () => {
    ...
    state._modalCtx = { tipo: tipoAtual, descAtual, cliAtual, dataAtual, valAtual, obsAtual };
...
function abrirModalNovoProduto(container, nomeInicial = "") {
  ...
  area.querySelector("#np-ok").addEventListener("click", async () => {
    ...
    if (catNome) {
      const { data: cats } = await supabase.from("categorias").select("id").ilike("nome", catNome).single();
...
async function abrirModalImportar(container) {
  ...
  area.querySelector("#modal-bg-imp").addEventListener("click", e => { if (e.target.id === "modal-bg-imp") area.innerHTML = ""; });
  area.querySelector("#imp-ok")?.addEventListener("click", async () => {
    ...
    const inserts = selecionadas.map(chk => ({
      tipo: "entrada", data: chk.dataset.data || state.filtroDia,
