...
function infoEntregue(entregueEm?: string | null): { texto: string; cor: string } | null {
  if (!entregueEm) return null;
  const dias = Math.floor((Date.now() - new Date(entregueEm).getTime()) / 86400000);
  const restam = 30 - dias;
  if (restam <= 5) return { texto: `some em ${Math.max(restam, 0)}d`, cor: 'text-red-400' };
  if (restam <= 10) return { texto: `some em ${restam}d`, cor: 'text-yellow-400' };
  return { texto: `há ${dias}d`, cor: 'text-gray-500' };
}
              ...
              >
                ...
                <div className="px-3.5 py-2 border-b border-gray-700 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold bg-gray-700 text-gray-300 px-1.5 py-0.5 rounded-full">
                      {cards.length}
                    </span>
                  </div>
                  <button
                    onClick={() => abrirNova(etapa.key)}
                    className="text-gray-500 hover:text-white w-5 h-5 flex items-center justify-center rounded hover:bg-gray-700 transition-all text-base leading-none"
                  >
                    +
                  </button>
                </div>
                ...
                <div className="p-2.5 space-y-2 min-h-20">
                  ...
                  {cards.map(ordem => {
                    ...
                    const infoEnt    = etapa.key === 'entregue' ? infoEntregue(ordem.entregue_em) : null;
                    ...
                    return (
                      <div
                        ...
                        className={`bg-[#1f2937] border border-gray-700 rounded-lg p-2.5 cursor-grab active:cursor-grabbing select-none transition-all hover:border-gray-500 hover:shadow-md ${
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mb-1.5">
                          ...
                          <div className="flex items-center gap-1.5">
                            {infoEnt && (
                              <span className={`text-[10px] font-bold flex items-center gap-1 ${infoEnt.cor}`}>
                                <Calendar className="w-3 h-3" /> {infoEnt.texto}
                              </span>
                            )}
                            {prazo && (
                        ...
                        {/* Título */}
                        <p className="text-xs font-bold text-white leading-snug mb-1">{ordem.titulo}</p>
                        ...
                        {/* Descrição */}
                        {ordem.descricao && (
                          <p className="text-[11px] text-gray-500 line-clamp-2 mb-1.5">{ordem.descricao}</p>
                        )}

                        {/* Footer: responsável + ações */}
                        <div className="flex items-center justify-between pt-1.5 border-t border-gray-700/50 mt-1">
                          ...
                          <div className="flex gap-1">
                            ...
                            <button
                              ...
                              className="flex items-center px-1.5 py-0.5 rounded bg-red-500/10 hover:bg-red-500/20 text-red-400 transition-all"
                            >
                              <X className="w-3 h-3" />
                            </button>
                          </div>
                        ...
                        {etapa.key !== 'entregue' && (
                          <button
                            ...
                            className="mt-1.5 w-full text-[10px] font-bold py-0.5 rounded-md border border-dashed border-gray-600 text-gray-500 hover:border-green-500/50 hover:text-green-400 hover:bg-green-500/5 transition-all flex items-center justify-center gap-1"
                          >
