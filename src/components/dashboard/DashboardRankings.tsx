...
export function DashboardRankings({
  ...
  data: any;
  produtoClassName?: string;
  clienteClassName?: string;
}) {
  return (
    <>
      {/* Top 5 Produtos */}
        ...
        <div className="flex-1 overflow-auto space-y-2.5">
          ...
          {(data?.top5Produtos ?? []).map((p: any, i: number) => (
            <div key={i} className="flex items-center justify-between gap-2 text-xs">
              ...
              <span className="font-bold text-white flex-shrink-0">{fmtBRL(p.total)}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="flex-1 overflow-auto space-y-2.5">
          ...
          {(data?.top5Clientes ?? []).map((c: any, i: number) => (
            <div key={i} className="flex items-center justify-between gap-2 text-xs">
              ...
              <span className="font-bold text-white flex-shrink-0">{fmtBRL(c.total)}</span>
            </div>
          ))}
        </div>
      </div>
