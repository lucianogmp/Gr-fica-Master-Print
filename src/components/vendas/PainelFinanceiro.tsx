...
export function PainelFinanceiro({
  ...
  const valorRestante    = Math.max(0, totalBase - valorPago);
  const quitado          = totalBase > 0 && valorRestante <= 0.01;
  const pctPago          = totalBase > 0 ? Math.min(100, (valorPago / totalBase) * 100) : 0;
    ...
    <div className="bg-[#1f2937] border border-gray-700 border-t-2 border-t-green-500 rounded-xl overflow-hidden">
      ...
      <div className="px-4 py-3 flex items-center gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <span className="text-[10px] text-gray-500 uppercase">Pagamento</span>
          <span className="text-xs font-black text-green-400">{fmtBRL(valorPago)}</span>
          {quitado ? (
        ...
        {/* Total */}
        <span className="text-xl font-black text-green-400 flex-shrink-0">{fmtBRL(totalBase)}</span>
        ...
        <div className="border-t border-gray-700/60 divide-y divide-gray-800">
          {pagamentos.map(p => (
            <div key={p.id} className="flex items-center gap-3 px-4 py-2 hover:bg-gray-800/20 transition-colors">
              ...
              <span className="text-sm font-black text-green-400 flex-shrink-0">{fmtBRL(p.valor)}</span>
